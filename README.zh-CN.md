...
1. **预算监控**：实时关注当前上下文消耗，在快触及预算上限前自动预留交接空间。
2. **安全切换**：必须等到当前命令执行结束、输入框处于空闲状态时才安全换窗；绝不会在任务运行中强行打断。
3. **无缝接力**：新窗口继承关键进度交接；旧窗口的所有对话在本地建有全文索引，随时能按原话搜回。
...
前置要求：Python 3.10+、Git、系统已安装 `tmux` 及原生 `claude` CLI。
...
当上下文接近预算阈值时，工具会在当前操作完成、终端静止后，自动完成交接并平滑换到新窗口，继续在原目录推进工作。
...
绝对不会。工具会一直等待当前所有命令执行完成、输入框完全处于空闲状态时才安全换窗；若无法明确判定状态，则保持原样不切。
